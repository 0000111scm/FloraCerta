...
class LocalPersistenceSnapshot {
  ...
  final List<ScanTrainingSample> scanSamples;
}
...
class LocalPersistenceService {
  ...
  static const _scanSamplesKey = 'scan_samples_v1';
  ...
  Future<LocalPersistenceSnapshot> loadAll() async {
    ...
    return LocalPersistenceSnapshot(
      ...
      scanSamples: await _decodeScanSamples(
        preferences.getString(_scanSamplesKey),
      ),
    );
  }
  ...
  Future<void> saveScanSamples(List<ScanTrainingSample> items) async {
    final preferences = await SharedPreferences.getInstance();
    final previousRaw = preferences.getString(_scanSamplesKey);
    final previousPaths = _extractStoredPaths(previousRaw);
    final encoded = <Map<String, dynamic>>[];

    for (final item in items) {
      final photoStoragePath = await _persistPhoto(
        category: 'scan_samples',
        id: item.id,
    ...
    await preferences.setString(_scanSamplesKey, jsonEncode(encoded));
    await _cleanupRemovedPaths(
      previousPaths: previousPaths,
      nextPaths: _extractStoredPathsFromMaps(encoded),
    );
  ...
  Future<List<ScanTrainingSample>> _decodeScanSamples(String? raw) async {
    if (raw == null || raw.isEmpty) {
      return <ScanTrainingSample>[];
    }

    final decoded = jsonDecode(raw) as List<dynamic>;
    final items = <ScanTrainingSample>[];

    for (final item in decoded) {
      final map = item as Map<String, dynamic>;
      final photoStoragePath = map['photoStoragePath'] as String?;
      final storedBytes = await _resolveStoredBytes(photoStoragePath);
      items.add(ScanTrainingSample.fromJson(map, resolvedPhotoBytes: storedBytes));
    }
    return items;
  }
